...
import { Button, Input, InputRef, message, Popover, Tooltip, TooltipProps } from 'antd';
...
export const ChatBoxIcon: FC<{ className?: string; width?: number; height?: number; fill?: string }> = ({
  className,
  width = 26,
  height = 28,
  fill = 'white',
...
export const ChatBox = forwardRef<ChatBoxRef, ChatBoxProps>((props, ref) => {
  ...
  const sendMessage = async (msg?: string) => {
    ...
    const newMsg = currentInput || msg;

    if (!newMsg || answerStatus === ChatBotAnswerStatus.Loading) {
      return;
    }
    ...
    try {
      ...
      const newChat = {
        ...curChat,
        messages: [...curChat.messages, newMessage].filter((i) => i.content),
        name: curChat.messages.length ? curChat.name : newMsg.slice(0, 40),
      };
      newChat.chatUrl = newChat.messages.length - 1 > 0 ? newChat.chatUrl : chatUrl;
      newChat.prompt = newChat.prompt || prompt || '';
